...
std::string Challenge1() {
    ...
    char funcName[] = { 'N','t','C','r','e','a','t','e','S','y','m','b','o','l','i','c','L','i','n','k','O','b','j','e','c','t',0 };
    ...
    // 2. 動態取得 NtCreateSymbolicLinkObject
    HMODULE hNtdll = GetModuleHandleA(ntdll);
    if (!hNtdll) return "ERR_NTDLL";

    PfnNtCreateSymbolicLinkObject pNtCreateSymbolicLinkObject =
        (PfnNtCreateSymbolicLinkObject)GetProcAddress(hNtdll, funcName);

    if (!pNtCreateSymbolicLinkObject) return "ERR_API";

    // 3. 建立 Object Manager Symlink (Directory Object)
    ...
    NTSTATUS status = pNtCreateSymbolicLinkObject(&hSymLink, SYMBOLIC_LINK_ALL_ACCESS, &oa, &usTargetName);

    // 如果失敗且不是名稱衝突，回傳錯誤
    if (!NT_SUCCESS(status) && status != 0x40000000) { // STATUS_OBJECT_NAME_EXISTS
        return ToHex(status);
    }

    // 4. 開啟 Key (透過 Symlink 路徑)
    ...
    // 用完 Symlink 就可以關閉 Handle，物件在參照計數歸零前(或我們明確刪除前)會短暫保留，
    // 或是我們可以等到 query 完再關。為了安全起見，我們先做 Query 再關閉 Symlink Handle。
    // 注意：RPC Control 下的物件是暫時的，Process 結束會消失，或我們可以使用 MakeTemporaryObject 刪除。

    if (!NT_SUCCESS(status)) {
        if (hSymLink) CloseHandle(hSymLink);
        return ToHex(status);
    }
    ...
    // 關閉 Symlink Handle (這會讓它在 Directory 中被移除，如果沒有其他 Handle 指向它)
    if (hSymLink) {
        // Optional: MakeTemporaryObject to ensure deletion
        // NtMakeTemporaryObject(hSymLink);
